...
This test patches `BatchSummarizationRunner.advance()`'s provider-check step
...
from pipeline.stages.summarization.batch import runner as runner_mod
...
def test_all_jobs_failed_raises_runtime_error(monkeypatch, tmp_path):
    ...
    mini = _MiniRunner.__new__(runner_mod.BatchSummarizationRunner)
    mini.handle_path = lambda _pmcid: tmp_path / "handle.json"

    with pytest.raises(RuntimeError) as exc_info:
        runner_mod.BatchSummarizationRunner.advance(mini, handle)
...
def test_partial_failure_emits_per_job_warning(monkeypatch, tmp_path, caplog):
    ...
    monkeypatch.setattr(
        runner_mod.BatchSummarizationRunner, "_process_level",
        lambda self, *a, **kw: None,
    )
    handle.save = lambda _path: None

    mini = runner_mod.BatchSummarizationRunner.__new__(runner_mod.BatchSummarizationRunner)
    mini.handle_path = lambda _pmcid: tmp_path / "handle.json"
    mini._enable_router = True
    mini._l3 = None
    mini._l2 = []

    with caplog.at_level(logging.WARNING, logger="pipeline.stages.summarization.batch.runner"):
        runner_mod.BatchSummarizationRunner.advance(mini, handle)
