...
def load_items(mode: str) -> list[Item]:
    loaders = {
...
def render(item: Item, total: int, ann: dict[str, str], mode: str) -> None:
    ...
    annotated   = n_correct + n_incorrect
    ...
    if key in ann:
        val    = ann[key]
        colour = GREEN if val == "correct" else (RED if val == "incorrect" else YELLOW)
